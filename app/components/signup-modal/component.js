...
export default Ember.Component.extend({
  ...
  actions: {
    ...
    expand: function(){
      this.$('.signup-modal-modal').modal('show');
    },
  ...
  /**
   * Call the facebook api for login requesting for the public profile, email, and friends.
   */
    ...
    if(response.status === 403) {
      if(response.responseJSON.statusMessage) {
        if(response.responseJSON.statusMessage == 'email duplication') {
          this.set('errorFlashMessage', 'The given email cannot be unsed twice');
      ...
      this.set('errorFalshMessage', 'We are experiencing intermitent problems on the server');
    }
  },
  /**
   ...
   */
   facebookSuccessAuthentication: function (fbResponse) {
    ...
    if(fbResponse.status == 'connected') {
      ...
      this.set('errorFlashMessage', 'There was a problem registering with facbeook ');
    }
   },
  /**
